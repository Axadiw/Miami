...
export const usePositionDetailsValidators = () => {
  ...
  return {
    slBelowOpen:
      calculatedValues && calculatedValues.slPercent < 0
        ? `SL should be ${side === 'Long' ? 'below' : 'above'} current price`
        : undefined,
    tp1AboveOpen:
      calculatedValues && calculatedValues.tp1Percent < 0
        ? `TP1 should be ${side === 'Long' ? 'above' : 'below'} current price`
        : undefined,
    tp2AboveTp1:
      calculatedValues && calculatedValues.tp1Percent >= calculatedValues.tp2Percent
        ? `TP2 should be ${side === 'Long' ? 'above' : 'below'} TP1`
        : undefined,
    tp3AboveTp2:
      calculatedValues && calculatedValues.tp2Percent >= calculatedValues.tp3Percent
    ...
    tpVolumesAddTo100:
      ...
      Number(tp1Percent) + Number(tp2Percent) + Number(tp3Percent) !== 100
        ...
        : undefined,
    maximumLossAbove0:
      calculatedValues && calculatedValues.maxLossUSD <= 0
        ? 'You need to specify maximum loss'
        : undefined,
    slAbove0:
      calculatedValues && calculatedValues.slPercent <= 0 ? 'SL need to be above 0%' : undefined,
    tp1Above0:
      calculatedValues && calculatedValues.tp1Percent <= 0 ? 'TP1 need to be above 0%' : undefined,
    tp2Above0:
