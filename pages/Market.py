...
st.write('Balance {:.2f} $'.format(bybit_balance))
symbol = symbol_value_col.text_input("Symbol")
direction = config_col.radio("direction", ['buy', 'sell'], horizontal=True)

max_loss = max_loss_value_col.text_input("Max loss")
tp1 = tp1_value_col.text_input("TP1")
tp2 = tp2_value_col.text_input("TP2")
tp3 = tp3_value_col.text_input("TP3")
sl = sl_value_col.text_input("SL")
...
if symbol:
    ...
    if tp1:
        calculated_tp1 = float(tp1) if tp1_type == '$' else symbol_current_price * ((float(tp1)) / 100 + 1)
        tp1_rendered_col.write('')
        tp1_rendered_col.write('')
        tp1_rendered_col.write(
            '{:.2f} $ ({:.2f} %)'.format(calculated_tp1, 100.0 * calculated_tp1 / symbol_current_price - 100))
    if tp2:
        calculated_tp2 = float(tp2) if tp2_type == '$' else symbol_current_price * ((float(tp2)) / 100 + 1)
        tp2_rendered_col.write('')
        tp2_rendered_col.write('')
        tp2_rendered_col.write(
            '{:.2f} $ ({:.2f} %)'.format(calculated_tp2, 100.0 * calculated_tp2 / symbol_current_price - 100))
    if tp3:
        calculated_tp3 = float(tp3) if tp3_type == '$' else symbol_current_price * ((float(tp3)) / 100 + 1)
        tp3_rendered_col.write('')
        tp3_rendered_col.write('')
        tp3_rendered_col.write(
            '{:.2f} $ ({:.2f} %)'.format(calculated_tp3, 100.0 * calculated_tp3 / symbol_current_price - 100))
    if sl:
        calculated_sl = float(sl) if sl_type == '$' else symbol_current_price * (1 - (float(sl)) / 100)
        sl_rendered_col.write('')
        sl_rendered_col.write('')
        sl_rendered_col.write(
            '{:.2f} $ ({:.2f} %)'.format(calculated_sl, abs((100.0 * calculated_sl / symbol_current_price) - 100)))
