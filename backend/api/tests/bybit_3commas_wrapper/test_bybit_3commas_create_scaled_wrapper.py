...
from api.endpoints.consts import UNKNOWN_3COMMAS_ERROR, SCALED_POSITION_CREATED
...
def test_should_pass_correct_params_to_py3w_with_3_tps(mocker):
    ...
    response = wrapper.create_scaled(side='Long', symbol='SYMBOL_NAME/USDT:USDT', position_size=10, lower_price=6,
                                     upper_price=9, orders_count=4,
    ...
    # assert py3w_spy.call_count == 4
    # assert response.json == SCALED_POSITION_CREATED
    ...
    #                                                        'units': {'value': 2.5}, "price": {"value": 9.0}},
    #                                           'stop_loss':
    #                                               {'breakeven': 'true',
...
def test_should_create_position_and_order_objects(app, mocker):
    ...
    wrapper.create_scaled(side='Long', symbol='SYMBOL_NAME/USDT:USDT', position_size=10, lower_price=6, upper_price=9,
                          orders_count=4,
                          take_profits=[[10, 30], [20, 70]],
                          stop_loss=5, comment='my_comment',
                          move_sl_to_breakeven_after_tp1=True,
    ...
    # assert stds[0].amount == 2.5
    # assert stds[0].position_id == positions[0].id
    #
    # assert stds[1].type == STANDARD_ORDER_TYPE
    # assert stds[1].state == CREATED_ORDER_STATE
    # assert stds[1].price == 7
    # assert stds[1].amount == 2.5
    # assert stds[1].position_id == positions[0].id
    #
    # assert stds[2].type == STANDARD_ORDER_TYPE
    # assert stds[2].state == CREATED_ORDER_STATE
    # assert stds[2].price == 8
    # assert stds[2].amount == 2.5
    # assert stds[2].position_id == positions[0].id
    #
    # assert stds[3].type == STANDARD_ORDER_TYPE
    # assert stds[3].state == CREATED_ORDER_STATE
    # assert stds[3].price == 9
    # assert stds[3].amount == 2.5
    # assert stds[3].position_id == positions[0].id
