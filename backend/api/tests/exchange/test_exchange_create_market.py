...
from api.tests.exchange_create_position_test_helpers import should_handle_properly_side, create_exchange_account, \
    should_handle_properly_symbol, should_handle_properly_take_profits, \
    should_handle_properly_stop_loss, should_handle_properly_soft_stop_loss, should_handle_properly_comment, \
    should_handle_properly_move_sl_to_be, fail_when_non_existing_account_provided, fail_for_account_from_different_user, \
    should_have_at_least_2tps_when_moveto_be_set, can_have_single_tp_when_dont_need_to_move_sl_to_be, \
    should_handle_properly_numeric_parameter

common_market_params = {'account_id': 1,
                        'symbol': 'SYMBOL_NAME',
                        'position_size': 10,
...
def test_should_handle_properly_side(client, mocker, user1_token):
    should_handle_properly_side(client, mocker, user1_token, common_market_params, 'create_market',
                                "/exchange_create_market_position")


def test_should_handle_properly_symbol(client, mocker, user1_token):
    should_handle_properly_symbol(client, mocker, user1_token, common_market_params, 'create_market',
                                  "/exchange_create_market_position")


def test_should_handle_properly_position_size(client, mocker, user1_token):
    should_handle_properly_numeric_parameter(client, mocker, user1_token, common_market_params, 'create_market',
                                             "/exchange_create_market_position", 'position_size')


def test_should_handle_properly_take_profits(client, mocker, user1_token):
    should_handle_properly_take_profits(client, mocker, user1_token, common_market_params, 'create_market',
                                        "/exchange_create_market_position")


def test_should_handle_properly_stop_loss(client, mocker, user1_token):
    should_handle_properly_stop_loss(client, mocker, user1_token, common_market_params, 'create_market',
                                     "/exchange_create_market_position")


def test_should_handle_properly_soft_stop_loss(client, mocker, user1_token):
    should_handle_properly_soft_stop_loss(client, mocker, user1_token, common_market_params, 'create_market',
                                          "/exchange_create_market_position")


def test_should_handle_properly_comment(client, mocker, user1_token):
    should_handle_properly_comment(client, mocker, user1_token, common_market_params, 'create_market',
                                   "/exchange_create_market_position")


def test_should_handle_properly_move_sl_to_be(client, mocker, user1_token):
    should_handle_properly_move_sl_to_be(client, mocker, user1_token, common_market_params, 'create_market',
                                         "/exchange_create_market_position")
...
def test_fail_when_non_existing_account_provided(client, user1_token):
    fail_when_non_existing_account_provided(client, user1_token, common_market_params, 'create_market',
                                            "/exchange_create_market_position")


def test_fail_for_account_from_different_user(client, user1_token, user2_token):
    fail_for_account_from_different_user(client, user1_token, user2_token, common_market_params, 'create_market',
                                         "/exchange_create_market_position")


def test_should_have_at_least_2tps_when_moveto_be_set(client, mocker, user1_token):
    should_have_at_least_2tps_when_moveto_be_set(client, mocker, user1_token, common_market_params, 'create_market',
                                                 "/exchange_create_market_position")


def test_can_have_single_tp_when_dont_need_to_move_sl_to_be(client, mocker, user1_token):
    can_have_single_tp_when_dont_need_to_move_sl_to_be(client, mocker, user1_token, common_market_params,
                                                       'create_market',
                                                       "/exchange_create_market_position")
